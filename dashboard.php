...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['create'])) {
    if (empty($_POST['nom']) || strlen($_POST['nom']) < 3 || strlen($_POST['nom']) > 50) {
        ...
        if (domaineNameExists($_POST['nom'])) {
            $errorMessage = "Un domaine avec ce nom existe déjà.";
        } else {
            // Proceed with file upload and adding the domain
            if (isset($_FILES['image']) && $_FILES['image']['error'] == UPLOAD_ERR_OK) {
                $targetDir = "../../view/backoff/Templates/uploads/";
                $targetFile = $targetDir . basename($_FILES["image"]["name"]);
                if (!is_dir($targetDir)) {
                ...
                if (move_uploaded_file($_FILES["image"]["tmp_name"], $targetFile)) {
                    addDomaines($_POST['nom'], $_POST['description'], $_POST['competence'], $targetFile);
                    header("Location: dashboard.php");
                    exit;
                } else {
                    $errorMessage = "Une erreur est survenue lors du téléchargement de l'image.";
...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['update'])) {
    ...
    if (empty($nom) || strlen($nom) < 3 || strlen($nom) > 50) {
        ...
        // Check if the new name already exists (excluding the current domain)
        if (domaineNameExists($nom, $id)) {
            $errorMessage = "Un domaine avec ce nom existe déjà.";
        } else {
            // Proceed with file upload and updating the domain
            if (isset($_FILES['image']) && $_FILES['image']['error'] == UPLOAD_ERR_OK) {
                $targetDir = "../../view/backoff/Templates/uploads/";
                $targetFile = $targetDir . basename($_FILES["image"]["name"]);
                if (!is_dir($targetDir)) {
            ...
            updateDomaines($id, $nom, $description, $competence, $image);
            header("Location: dashboard.php");
            exit;
        }
    }
