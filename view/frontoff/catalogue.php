    ...
    <div class="container-fluid">
        ...
        <div class="collapse navbar-collapse" id="navbarCollapse">
            <div class="navbar-nav mx-auto py-0">
                ...
                <select id="language-selector" class="nav-item nav-link" style="background: none; border: none; color: white;">
                    <option value="fr" style="color: black;">Français</option>
                    <option value="en" style="color: black;">English</option>
                </select>
          <!-- Add these buttons inside the <body> tag, preferably near the title or header -->
...
<form method="GET" class="mb-4">
    ...
    <form method="POST" class="mb-4">
        <div class="input-group">
            <input type="text" name="search" class="form-control" placeholder="Rechercher un cours..." value="<?php echo htmlspecialchars($search_query); ?>">
    ...
    <?php if ($domaine_id > 0): ?>
        <h2 class='text-center'>Cours pour le domaine "<?php echo htmlspecialchars(getDomaineById($domaine_id)['nom']); ?>"</h2>
    <?php endif; ?>
    ...
    <!-- Add this HTML structure after your existing search form -->
<div class="advanced-search-container">
    <div class="search-box">
        <div class="input-group">
            <input type="text" id="searchInput" class="form-control" placeholder="Rechercher par voix...">
    ...
    <div class='text-center mb-4'>
        <a href='flashcards.php' class='btn btn-primary'>Accéder aux Flashcards</a>
    </div>
                ...
                <div class='col-md-4 mb-4'>
                    ...
                    <div class='domaine-card shadow-sm p-4 rounded text-center'>
                        <?php if (!empty($domaine['image'])) : ?>
                            ...
                            <img src="<?php echo htmlspecialchars($domaine['image']); ?>" alt="<?php echo htmlspecialchars($domaine['nom']); ?>" />
                        <?php else : ?>
...
<script>
    ...
    const translations = {
        'en': {
            ...
            // Main content
            "Les domaines disponibles": "Available Domains",
            "Sélectionner un domaine": "Select a domain",
            "Afficher Cours": "Show Courses",
            "Rechercher un cours...": "Search for a course...",
            "Rechercher": "Search",
            "Cours pour le domaine": "Courses for domain",
            "Télécharger le cours": "Download course",
            "Aucun cours trouvé pour votre recherche.": "No courses found for your search.",
            "Aucun cours trouvé pour ce domaine.": "No courses found for this domain.",
            "Accéder aux Flashcards": "Access Flashcards",
            "Voir Cours": "View Courses",
            "Aucun domaine disponible.": "No domains available.",
        ...
        'fr': {
            // French translations (original text)
            "Home": "Accueil",
            "Catalog": "Catalogue",
    ...
    function translateText(element, targetLang) {
        if (element.hasChildNodes()) {
            element.childNodes.forEach(child => {
                if (child.nodeType === 3) { // Text node
                    let text = child.textContent.trim();
                    if (text && translations[targetLang][text]) {
                        child.textContent = child.textContent.replace(text, translations[targetLang][text]);
                    }
                } else {
                    translateText(child, targetLang);
                }
            });
        }
    }
    ...
    function translateAttributes(element, targetLang) {
        if (element.hasAttributes()) {
            ['placeholder', 'title'].forEach(attr => {
                if (element.hasAttribute(attr)) {
                    const value = element.getAttribute(attr);
                    if (translations[targetLang][value]) {
                        element.setAttribute(attr, translations[targetLang][value]);
                    }
                }
            });
        }
    ...
    function translatePage(targetLang) {
        translateText(document.body, targetLang);
        document.querySelectorAll('*').forEach(element => {
            translateAttributes(element, targetLang);
        });
    ...
    speakButton.addEventListener('click', function() {
        if (isReading) {
            ...
            // Récupérer le contenu principal à lire
            ...
            utterance.lang = document.getElementById('language-selector').value === 'fr' ? 'fr-FR' : 'en-US';
            utterance.rate = 1;
            utterance.pitch = 1;
