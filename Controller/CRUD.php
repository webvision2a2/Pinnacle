...
function getRatingStatistics() {
    ...
    try {
        ...
        $query = "SELECT d.nom, COALESCE(AVG(r.rating), 0) as avg_rating, COUNT(r.id) as total_ratings
                  FROM domaines d
                  LEFT JOIN ratings r ON d.id = r.domaine_id
                  GROUP BY d.id, d.nom";
        
        $stmt = $db->query($query);
        $stats['domainRatings'] = $stmt->fetchAll(PDO::FETCH_ASSOC);
        
        // Overall average rating
        $query = "SELECT COALESCE(AVG(rating), 0) as overall_rating FROM ratings";
        $stmt = $db->query($query);
        $stats['overallRating'] = $stmt->fetch(PDO::FETCH_ASSOC)['overall_rating'];
        ...
        $query = "SELECT rating, COUNT(*) as count 
                  ...
                  ORDER BY rating";
        $stmt = $db->query($query);
        $stats['ratingDistribution'] = $stmt->fetchAll(PDO::FETCH_ASSOC);
        
        return $stats;
        
    } catch (PDOException $e) {
        // Return default values if there's an error or no ratings yet
        return [
            'domainRatings' => [],
            'overallRating' => 0,
            'ratingDistribution' => [
