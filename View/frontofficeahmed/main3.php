...
require_once '../../config_zeineb.php'; // Inclure votre fichier config pour la connexion à la base de données
...
if (isset($_POST['submit'])) {
    $id_user = $_SESSION['id']; // L'ID utilisateur est fixe à 1 dans cet exemple

    // Vérifier si l'utilisateur a déjà postulé pour ce stage
    $stmt = $db->prepare("SELECT * FROM candidatures WHERE id_user = ? AND id_stage = ?");
    $stmt->execute([$id_user, $id_stage]);

    if ($stmt->rowCount() > 0) {
        // L'utilisateur a déjà postulé pour ce stage
        echo '<div class="alert alert-danger" role="alert">
                Vous avez déjà postulé pour ce stage. Vous ne pouvez postuler qu\'une seule fois.
              </div>';
    } else {
        ...
        if ($cv['error'] == 0) {
            ...
            $stmt = $db->prepare("INSERT INTO candidatures (nom, prenom, numero, email, cv, id_stage, id_user) VALUES (?, ?, ?, ?, ?, ?, ?)");
            if ($stmt->execute([$nom, $prenom, $numero, $email, $cv_path, $id_stage, $id_user])) {
                // Message de succès
